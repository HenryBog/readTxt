...
// 动画时间
#define AnimateDuration 0.30


#import "KPageViewController.h"

@interface KPageViewController ()<UIGestureRecognizerDelegate>
    ...
    if (self.pendingController) {
        ...
        if (self.isLeft) { // 左边
            ...
            if (animated) {
                ...
                __weak KPageViewController *weakSelf = self;
...
- (UIViewController * _Nullable)GetTapControllerWithTouchPoint:(CGPoint)touchPoint {
    ...
    if ((touchPoint.x < SpaceWith * 2 && touchPoint.y < SpaceHeight) || (touchPoint.x < SpaceWith && touchPoint.y > SpaceHeight)) { // 左边
        self.isLeft = YES;
    ...
    else if ((touchPoint.x > SpaceWith * 2 && touchPoint.y < SpaceHeight * 2) || (touchPoint.x > SpaceWith && touchPoint.y > SpaceHeight * 2)) { //右边
        ...
        if ([self.delegate respondsToSelector:@selector(KPageViewControllerTapWithMenu)]) {
            [self.delegate KPageViewControllerTapWithMenu];
        }
    }
