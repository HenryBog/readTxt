...
+(void)separateChapter:(NSMutableArray **)chapters model:(RTBookDirectoryModel *)model {
    [*chapters removeAllObjects];
//    NSArray *array = [[RTBookCacheManager shareManager] getRTBookFileWithBookid:model.book_id];
    NSArray *array = [NSArray arrayWithObject:model];
    NSMutableArray *chap = [NSMutableArray array];
    [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        RTBookDirectoryModel *bookModel = obj;
        if (bookModel && bookModel.chapter_title) {
            LSYChapterModel *model = [[LSYChapterModel alloc] init];
            model.title = bookModel.chapter_title;
            if (bookModel.bookDetailModel && bookModel.bookDetailModel.content) {
                model.content = bookModel.bookDetailModel.content;
            } else {
                model.content = @"";
            }
            [chap addObject:model];
        }
    }];
    [*chapters addObjectsFromArray:chap];
}
